...
void setup()
{
  ...
  // Timer/Counter1 Control Registers A and B, used to configure Timer1.
  ...
  //TIMSK1: Timer/Counter1 Interrupt Mask Register, used to 
  //enable or disable Timer1 interrupts.
  TIMSK1 |= B00000010;
  /*4. Set the value of register A to 31250*/
  OCR1A = 31250; // Finally we set compare register A to this value
  sei();//enables the timer interrupt to be triggered when the timer counter reaches the value
...
ISR(TIMER1_COMPA_vect)
{
  ...
  if(isTime)
  {
  	digitalWrite(TIMER_LED, !digitalRead(TIMER_LED)); // Update LED state
  	counter = 0;
    isTime = false;
  }
  counter++;
